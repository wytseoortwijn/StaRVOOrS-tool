...
makeTransitions c n esinf es = 
   ...
   in if (length xs == 1)
      then let idle_to_postok = Arrow event ("Contracts." ++ consName ++ "_post(" ++ init arg ++ ")") ("System.out.println(\"    " ++ consName ++ "_postOK \\n \");")
               idle_to_bad    = Arrow event ("!Contracts." ++ consName ++ "_post(" ++ init arg ++ ")") ("System.out.println(\"    " ++ consName ++ "_bad \\n \");")   
